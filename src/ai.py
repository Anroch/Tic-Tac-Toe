...
class TicTacToeAI:
    ...
    def main(self):
        ...
        while True:
            ...
            current_board_state = detector.detect_board(frame) # Detecta el estado actual

            # Si se detecta un tablero válido y es diferente al anterior (hubo un cambio)
            if current_board_state is not None and current_board_state != self.previous_board:
                 print("Tablero detectado:", current_board_state)
                 # Comprobar si el jugador hizo un movimiento válido (solo un cambio)
                 # (Esta lógica puede necesitar ajustes dependiendo de cómo detect_board actualiza)

                 # Turno de la IA
                 move = self.next_move(current_board_state)
                 if move:
                     index, player_symbol = move
                     print(f"IA decide jugar en {index}")
                     # Aquí necesitarías una forma de actualizar el tablero 'real'
                     # o indicar el movimiento de la IA.
                     # Por ahora, actualizamos nuestro estado interno para la siguiente iteración.
                     current_board_state[index] = player_symbol
                     self.previous_board = current_board_state.copy() # Actualizar el estado previo

                 else:
                     print("IA no puede moverse (tablero lleno o sin movimientos válidos).")


            # Verificar si alguien ganó después del posible movimiento de la IA
            if current_board_state: # Solo si tenemos un estado de tablero
                winner = self.check_win(current_board_state)
                if winner != BoardSlot.Empty and winner != BoardSlot.Tie:
                    print(f"Ganó: {winner.name}")
                    break
                elif winner == BoardSlot.Tie:
                    print("Empate")
                    break

            # Actualizar el previous_board si no hubo movimiento de IA pero sí cambio detectado
            elif current_board_state is not None and current_board_state != self.previous_board:
                 self.previous_board = current_board_state.copy()


            # Mostrar la imagen (opcional, si tienes la visualización implementada)
            # if self.show_detections and frame is not None:
            #    cv2.imshow('board', frame) # Muestra el frame original
            if cv2.waitKey(1) & 0xff == ord('q'):
                break

        detector.cam.release() # Liberar la cámara al salir
